...
function s7upf_like_scripts() {
	wp_enqueue_script('s7upf_like_post', plugins_url() . '/7up-core/assets/js/post-like.js', array( 'jquery' ),null,true);
	wp_localize_script( 's7upf_like_post', 'ajax_var', array(
		'url' => admin_url( 'admin-ajax.php' ),
		'nonce' => wp_create_nonce( 'ajax-nonce' )
		)
	);
}
add_action( 'init', 's7upf_like_scripts' );

/**
 * (2) Save like data
 */
add_action( 'wp_ajax_nopriv_sv-post-like', 's7upf_post_like' );
add_action( 'wp_ajax_sv-post-like', 's7upf_post_like' );
function s7upf_post_like() {
	$nonce = $_POST['nonce'];
    if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) )
        die ( 'Nope!' );
	
	if ( isset( $_POST['s7upf_post_like'] ) ) {
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( !s7upf_AlreadyLiked( $post_id ) ) { // like the post
 ...
 */
function s7upf_AlreadyLiked( $post_id ) { // test if user liked before
	if ( is_user_logged_in() ) { // user is logged in
		$user_id = get_current_user_id(); // current user
		$meta_USERS = get_post_meta( $post_id, "_user_liked" ); // user ids from post meta
 ...
 */
add_action( 'show_user_profile', 's7upf_show_user_likes' );
add_action( 'edit_user_profile', 's7upf_show_user_likes' );
function s7upf_show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php esc_html_e( 'You Like:','fashionstore' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . esc_html_e( 'You don\'t like anything yet.','fashionstore' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
 ...
 */
function s7upf_frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		$like_list = '';
		$user_id = get_current_user_id(); // current user
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__( 'You Like:','fashionstore' ) . "</h3>\n";
			$like_list .= "<ul>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function s7upf_most_popular_today() {
	global $post;
	$today = date('j');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'Today\'s Most Popular Posts','fashionstore' ) . "</h3>\n";
		echo "<ul>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function s7upf_most_popular_month() {
	global $post;
	$month = date('m');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Month\'s Most Popular Posts','fashionstore' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . esc_url(get_permalink($post->ID)) . "'>" . get_the_title() . "</a></li>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function s7upf_most_popular_week() {
	global $post;
	$week = date('W');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html_e( 'This Week\'s Most Popular Posts','fashionstore' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . esc_url(get_permalink($post->ID)) . "'>" . get_the_title() . "</a></li>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function s7upf_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . esc_html_e( 'Most Popular Posts','fashionstore' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_query' => array(
