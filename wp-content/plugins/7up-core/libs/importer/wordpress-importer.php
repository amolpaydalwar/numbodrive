...
<?php
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			$thumbnail = isset( $term['product_cat_thumbnail_id'] ) ? $term['product_cat_thumbnail_id'] : '';
			$header_image = isset( $term['term_meta[header_cat]'] ) ? $term['term_meta[header_cat]'] : '';
			$termarr = array( 'slug' => $term['slug'], 'description' => $description, 'parent' => intval($parent), 'product_cat_thumbnail_id' => $thumbnail, 'term_meta[header_cat]' => $header_image);
	...
	function process_menu_item( $item ) {
		...
		foreach ( $item['postmeta'] as $meta ){
			$$meta['key'] = $meta['value'];
			if($meta['key'] == '_menu_item_type' && !isset($_menu_item_type)) $_menu_item_type = $meta['value'];
			if($meta['key'] == '_menu_item_classes' && !isset($_menu_item_classes)) $_menu_item_classes = $meta['value'];
			if($meta['key'] == '_menu_item_object_id' && !isset($_menu_item_object_id)) $_menu_item_object_id = $meta['value'];
			if($meta['key'] == '_menu_item_object' && !isset($_menu_item_object)) $_menu_item_object = $meta['value'];
			if($meta['key'] == '_menu_item_menu_item_parent' && !isset($_menu_item_menu_item_parent)) $_menu_item_menu_item_parent = $meta['value'];
			if($meta['key'] == '_menu_item_url' && !isset($_menu_item_url)) $_menu_item_url = $meta['value'];
			if($meta['key'] == '_menu_item_target' && !isset($_menu_item_target)) $_menu_item_target = $meta['value'];
			if($meta['key'] == '_menu_item_xfn' && !isset($_menu_item_xfn)) $_menu_item_xfn = $meta['value'];
		}
		...
		$args = array(
			...
			// 'enable_megamenu' 	=> $enable_megamenu,
			// 'icon_menu1' 		=> $icon_menu1,
			// 'icon_menu2' 		=> $icon_menu2,
			// 'content1'			=> $content1,
			// 'content2'			=> $content2,
			// 'col_size'			=> $col_size
		);
		if(isset($enable_megamenu)) $args['enable_megamenu'] = $enable_megamenu;
		if(isset($icon_menu1)) $args['icon_menu1'] = $icon_menu1;
		if(isset($icon_menu2)) $args['icon_menu2'] = $icon_menu2;
		if(isset($content0)) $args['content0'] = $content0;
		if(isset($content1)) $args['content1'] = $content1;
		if(isset($content2)) $args['content2'] = $content2;
		if(isset($col_size)) $args['col_size'] = $col_size;

		$id = st_wp_update_nav_menu_item( $menu_id, 0, $args );
...
function st_wp_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
    $menu_id = (int) $menu_id;
    ...
    if(isset($args['icon_menu1'])) update_post_meta( $menu_item_db_id, 'icon_menu1', $args['icon_menu1'] );
    if(isset($args['icon_menu2'])) update_post_meta( $menu_item_db_id, 'icon_menu2', $args['icon_menu2'] );
    if(isset($args['content0'])) update_post_meta( $menu_item_db_id, 'content0', $args['content0'] );
    if(isset($args['content1'])) update_post_meta( $menu_item_db_id, 'content1', $args['content1'] );
    if(isset($args['content2'])) update_post_meta( $menu_item_db_id, 'content2', $args['content2'] );
    if(isset($args['col_size'])) update_post_meta( $menu_item_db_id, 'col_size', $args['col_size'] );
