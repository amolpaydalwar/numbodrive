...
function ywpc_install_free_admin_notice() {
    ?>
    <div class="error">
        <p><?php _e( 'You can\'t activate the free version of YITH WooCommerce Product Countdown while you are using the premium one.', 'ywpc' ); ?></p>
    </div>
<?php
}
...
function ywpc_install_woocommerce_admin_notice() {
    ?>
    <div class="error">
        <p><?php _e( 'YITH WooCommerce Product Countdown is enabled but not effective. It requires WooCommerce in order to work.', 'ywpc' ); ?></p>
    </div>
<?php
}
...
function ywpc_free_init() {
    ...
    /* Load text domain */
    load_plugin_textdomain( 'ywpc', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
