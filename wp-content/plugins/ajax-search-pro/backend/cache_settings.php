...
<div id="wpdreams" class='wpdreams wrap<?php echo isset($_COOKIE['asp-accessibility']) ? ' wd-accessible' : ''; ?>'>
	...
	<div class="wpdreams-box" style="float:left;">
		...
		<div class="item">
			...
			<?php $o = new wpdreamsYesNo( "caching", __('Caching activated', 'ajax-search-pro'), $cache_options["caching"]); ?>
			<p class="descMsg">
                <?php echo __('This will enable search results to be cached into files in the cache directory to bypass database query. Useful if you experience many repetitive queries.', 'ajax-search-pro'); ?>
            </p>
		</div>
		<div class="item">
			<p class='infoMsg'>
			...
			jQuery(function ($) {
				...
				$('#clearcache').on('click', function () {
					...
					$.post(ajaxsearchpro.ajaxurl, data, function (response) {
						...
						var cleared = '<?php echo esc_html__('%s file(s) deleted at %s', 'ajax-search-pro'); ?>';
