...
if ( ! class_exists( 'YITH_WCDP_Frontend' ) ) {
	...
	class YITH_WCDP_Frontend {
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 'yith-wcdp', $template_js, array( 'jquery', 'wc-add-to-cart-variation', 'jquery-blockui', 'selectWoo', 'wc-country-select', 'wc-address-i18n' ), YITH_WCDP::YITH_WCDP_VERSION, true );
			...
			if( is_product() || ( is_object( $post ) && isset( $post->post_content ) && false !== strpos( $post->post_content, '[product_page' ) ) ){
				...
				wp_localize_script( 'yith-wcdp', 'yith_wcdp', array(
					...
					'variations_handling' => defined( 'YITH_WCDP_Premium' ) && YITH_WCDP_Premium
				) );
			}
		...
		public function add_deposit_item_data( $data, $cart_item ) {
			if ( isset( $cart_item['deposit'] ) && $cart_item['deposit'] && $cart_item['deposit_value'] ) {
				$full_amount =  $cart_item['deposit_value'] + $cart_item['deposit_balance'];

				$full_amount_item = array(
					'display' => wc_price( yith_wcdp_get_price_to_display( $cart_item['data'], array( 'qty' => intval( $cart_item['quantity'] ), 'price' => $full_amount ) ) ),
				...
				$balance_item = array(
					'display' => wc_price( yith_wcdp_get_price_to_display( $cart_item['data'], array( 'qty' => intval( $cart_item['quantity'] ), 'price' => $cart_item['deposit_balance'] ) ) ),
		...
		public function filter_cart_total( $total_html ) {
			...
			if( ! empty( $cart_contents ) ) {
				foreach ( $cart_contents as $cart_item ){
					if( isset( $cart_item['deposit'] ) && $cart_item['deposit'] ){
						$balance_total += $cart_item['deposit_balance'];
					}
				}
			}

			if( $balance_total ) {
				$total = WC()->cart->get_total( 'edit' ) + $balance_total;
		...
		public function filter_order_status( $status, $order ) {
			if( is_account_page() && yit_get_prop( $order, '_has_deposit' ) && in_array( $status, array( 'completed', 'processing' ) ) ){
