...
if ( ! class_exists( 'YITH_WCDP_Admin' ) ) {
	...
	class YITH_WCDP_Admin {
		...
		public function __construct() {
			...
			add_action( 'save_post', array( $this, 'save_bulk_editing_fields' ), 10, 1 );
		...
		public function action_links( $links ) {
			$plugin_links = array(
				'<a href="' . admin_url( 'admin.php?page=yith_wcdp_panel&tab=settings' ) . '">' . __( 'Settings', 'yith-woocommerce-deposits-and-down-payments' ) . '</a>'
			);

			if( ! defined( 'YITH_WCDP_PREMIUM_INIT' ) ){
				$plugin_links[] = '<a target="_blank" href="' . $this->get_premium_landing_uri() . '">' . __( 'Premium Version', 'yith-woocommerce-deposits-and-down-payments' ) . '</a>';
				$plugin_links[] = '<a target="_blank" href="' . $this->live_demo_url . '">' . __( 'Live Demo', 'yith-woocommerce-deposits-and-down-payments' ) . '</a>';
			}
		...
		public function add_plugin_meta( $plugin_meta, $plugin_file ){
			if ( $plugin_file == plugin_basename( YITH_WCDP_DIR . 'init.php' ) ) {
				// documentation link
				$plugin_meta['documentation'] = '<a target="_blank" href="' . $this->doc_url . '">' . __( 'Plugin Documentation', 'yith-woocommerce-deposits-and-down-payments' ) . '</a>';
			}
						...
						foreach ( $suborder_ids as $suborder_id ) {
							...
							if( ! empty( $items ) ){
								foreach( $items as $item ){
									$items_to_string[] = sprintf( '<dt>%s</dt><dd>%s</dd>', $item['name'], wc_price( $suborder->get_item_total( $item ) ) );
								}
							}

							$items_to_string = '<dl class="suborder-items">' . implode( '', $items_to_string ) . '</dl>';

							printf( '<div class="suborder-details"><mark class="%s tips" data-tip="%s">%s</mark><div class="suborder-data"><strong><a href="%s">#%s</a></strong> %s</div></div>',
