 	...
 	function start_el(  &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
	    ...
	    $indent = ( $depth > 0 ? str_repeat( "\t", $depth ) : '' ); // code indent
	  	...
	  	if(!empty($content_item)){
			//$content = S7upf_Template::get_vc_pagecontent($content_item);
			$page=get_post($content_item);
	    ...
	    if(!empty($image))  $item_output .= '<div class="preview-image">
  		...
  		if($mega_menu){
    		...
    		if($depth == 0) $output .= '<div class="mega-menu" style="width:'.esc_attr($width).'">'.do_shortcode($content).'</div>';
    	}	
