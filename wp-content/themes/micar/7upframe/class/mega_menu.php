...
if($s7upf_config['mega_menu'] == '1'){
    class S7upf_CustomMenu
    {
        ...
        static function init()
        {
            self::$allFields=array(
                'enable_megamenu'=>array(
                    'label'=>esc_html__('Enable Mega menu','micar'),
                    'type'=>'checkbox',
                    'depth'=>0,
                ),
                'icon_menu2'=>array(
                    'label'=>esc_html__('Custom width Mega menu. Example "500px".','micar'),
                    'type'=>'text',
                ),
                'image'=>array(
                    'label'=>esc_html__('Image Preview','micar'),
                    'type'=>'image',
                ),
                'icon_menu1'=>array(
                    'label'=>esc_html__('Icon Menu','micar'),
                    'type'=>'text',
                    'class'=>'sv_iconpicker',
                ),
                'content2'=>array(
                    'label'=>esc_html__('Content From Mega Item','micar'),
                    'type'=>'select',
                    'depth'=>0,
                    'choices'=> s7upf_list_post_type('s7upf_mega_item',false)
                ),
                'content1'=>array(
                    'label'=>esc_html__('Simple Content','micar'),
                    'type'=>'text_html',
                    'depth'=>0,
                ),
        ...
        static function _add_scripts()
        {
            ...
            {
                wp_register_script( 'st-jspicker', get_template_directory_uri().'/js/colpick.js', array( 'jquery' ),'', true );
                wp_enqueue_script('st-jspicker');
                wp_register_script( 'st-custompicker', get_template_directory_uri().'/js/custompicker.js', array( 'jquery' ),'', true );
                wp_enqueue_script('st-custompicker');

                wp_enqueue_style( 'st-colorpicker', get_template_directory_uri().'/css/colpick.css' );
            }
        }
        ...
        static function _field_type_image($item,$key,$value,$d=0){
            ...
            <p class="field-custom description description-wide">
                ...
                <div class="wrap-metabox">
                    ...
                    <a class="button button-primary sv-button-upload-id"><?php esc_html_e("Upload","micar")?></a>
    ...
    class S7upf_WalkerNavMenu extends Walker_Nav_Menu
    {
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( esc_html__( '%s (Invalid)' ,'micar'), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                /* translators: %s: title of menu item in draft status */
                $title = sprintf( esc_html__('%s (Pending)','micar'), $item->title );
            }
            ...
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" style="<?php echo esc_attr($submenu_text); ?>"><?php esc_html_e( 'sub item' ,'micar'); ?></span></span>
                        <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','micar'); ?>">&#8593;</abbr></a>
                                |
                                <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
                                ...
                                ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','micar'); ?>">&#8595;</abbr></a>
                            </span>
                            <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','micar'); ?>" href="<?php
                            ...
                            ?>"><?php esc_html_e( 'Edit Menu Item' .'micar'); ?></a>
                        </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Navigation Label' ,'micar'); ?><br />
                        <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                    </label>
                </p>
                <p class="description description-thin">
                    <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Title Attribute' ,'micar'); ?><br />
                ...
                <p class="field-link-target description">
                    <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Open link in a new window/tab','micar' ); ?>
                    </label>
                </p>
                <?php
                ...
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'CSS Classes (optional)' ,'micar'); ?><br />
                ...
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Link Relationship (XFN)' ,'micar'); ?><br />
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Description' ,'micar'); ?><br />
                        <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                        <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.' ,'micar'); ?></span>
                    </label>
                </p>

                <p class="field-move hide-if-no-js description description-wide">
                    <label>
                        <span><?php esc_html_e( 'Move' ,'micar'); ?></span>
                        <a href="#" class="menus-move-up"><?php esc_html_e( 'Up one' ,'micar'); ?></a>
                        <a href="#" class="menus-move-down"><?php esc_html_e( 'Down one' ,'micar' ); ?></a>
                        <a href="#" class="menus-move-left"></a>
                        <a href="#" class="menus-move-right"></a>
                        <a href="#" class="menus-move-top"><?php esc_html_e( 'To the top' ,'micar'); ?></a>
                    </label>
                </p>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( esc_html__('Original: %s','micar'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php esc_html_e( 'Remove' ,'micar'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel','micar'); ?></a>
                </div>
